...
export const GamesSection: React.FC<GamesSectionProps> = ({
  ...
  const [localUserStats, setLocalUserStats] = useState<LocalStats>(() => {
    try {
  ...
  const userStats: LocalStats = externalStats
    ? {
        ...DEFAULT_STATS,
        ...externalStats,
  ...
  const addRewards = (xpAmount: number, coinAmount: number, gameId?: string, score?: number) => {
    ...
    setLocalUserStats(updatedStats);
    try {
      localStorage.setItem('animal_vision_user_stats', JSON.stringify(updatedStats));
    } catch (e) {
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 pb-24 animate-fade-in">
      ...
      {activeSubTab === 'active_game' && (
        <div className="bg-slate-900/90 border border-emerald-500/30 rounded-3xl p-6 shadow-2xl">
          ...
          {activeGameId === 'memory_card' && <MemoryCardGame onGameEnd={handleMiniGameEnd} onBack={() => setActiveSubTab('dashboard')} />}
        </div>
      )}
