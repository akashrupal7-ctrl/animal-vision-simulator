...
${animalContext ? `Current Selected Animal Context: ${JSON.stringify(animalContext)}` : ""}` : defaultSystemInstruction;
      const rawMessages = Array.isArray(messages) ? messages : [];
      const validMessages = rawMessages.filter(
        (m) => m && typeof m.content === "string" && m.content.trim().length > 0
      );
      if (ai) {
        let formattedContents = validMessages.map((m) => ({
...
\u2022 **Field of View & Acuity:** ${animal?.stats?.fovDegrees || 200}\xB0 FOV with ${animal?.stats?.acuitySnellen || "20/50"} acuity.
        ...
        const response = await ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                animalAName: { type: import_genai.Type.STRING },
                animalBName: { type: import_genai.Type.STRING },
                table: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      animalASpec: { type: import_genai.Type.STRING },
                      animalBSpec: { type: import_genai.Type.STRING },
                      winnerOrAdvantage: { type: import_genai.Type.STRING }
                    }
                  }
                },
                scientificOverview: { type: import_genai.Type.STRING },
                keyEvolutionaryInsight: { type: import_genai.Type.STRING }
              }
            }
          }
        });
        ...
        comparison: {
          ...
          table: [
            ...
            },
            {
              ...
              animalASpec: `${animalA.stats?.fovDegrees || 200}\xB0`,
              animalBSpec: `${animalB.stats?.fovDegrees || 200}\xB0`,
            ...
            responseSchema: {
              ...
              items: {
                ...
                properties: {
                  ...
                  animalContext: { type: import_genai.Type.STRING }
                },
        ...
        const response = await ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: {
              type: import_genai.Type.OBJECT,
              properties: {
                currentAnimal: { type: import_genai.Type.STRING },
                recommendations: {
                  type: import_genai.Type.ARRAY,
                  items: {
                    type: import_genai.Type.OBJECT,
                    properties: {
                      ...
                      scientificName: { type: import_genai.Type.STRING },
                      category: { type: import_genai.Type.STRING },
                      keyVisualTrait: { type: import_genai.Type.STRING },
                      whyStudy: { type: import_genai.Type.STRING }
                    }
                  }
                }
              }
...
Animal Profiles Summary: ${JSON.stringify(
          ...
          config: {
            ...
            responseSchema: {
              ...
              items: {
                type: import_genai.Type.OBJECT,
                properties: {
                  animalId: { type: import_genai.Type.STRING },
                  animalName: { type: import_genai.Type.STRING },
      ...
      const filtered = (animalsDatabase || []).filter((a) => {
  ...
  app.post("/api/tts", async (req, res) => {
    try {
      ...
      const cleanText = text.replace(/\*\*(.*?)\*\*/g, "$1").replace(/\*(.*?)\*/g, "$1").replace(/`(.*?)`/g, "$1").replace(/#+\s/g, "").replace(/\[(.*?)\]\(.*?\)/g, "$1").replace(/[•\-\*]\s/g, ". ");
      if (ai) {
        const response = await ai.models.generateContent({
...
function generateFallbackAnimalVisionReply(prompt, animalContext) {
...
\u2022 **Field of View:** ${animalContext?.stats?.fovDegrees || 300}\xB0 angular range with ${animalContext?.stats?.acuitySnellen || "20/50"} acuity.
